CREATE OR REPLACE FUNCTION sync_document_metadata() RETURNS TRIGGER AS $$
        DECLARE
          v_val TEXT;
        BEGIN
    ...
    BEGIN
      ...
      IF (NEW.data->'initialStockLevels') IS NOT NULL THEN
        FOR v_cid, v_qty IN SELECT * FROM jsonb_each_text(NEW.data->'initialStockLevels') LOOP
           ...
           VALUES ('mov-init-' || NEW.id || '-' || v_cid, v_cid, NEW.id, 'wh-' || v_cid, 'IN', v_qty::NUMERIC, NEW.id, 'OPENING_STOCK', COALESCE(NULLIF(NEW.data->>'createdAt', '')::DATE, NOW()::DATE), v_cost)
