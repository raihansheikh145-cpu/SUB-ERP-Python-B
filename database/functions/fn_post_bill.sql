...
BEGIN
    ...
    WHERE (journal_number = v_bill.bill_number OR reference_number = v_bill.bill_number) AND company_id = p_company_id LIMIT 1;
    ...
    SELECT id::text INTO v_vendor_account FROM docs_accounts WHERE code = '200100' AND company_id::text = p_company_id LIMIT 1;
    SELECT id::text INTO v_expense_account FROM docs_accounts WHERE code = '500100' AND company_id::text = p_company_id LIMIT 1;
    ...
    IF v_vendor_account IS NOT NULL AND v_expense_account IS NOT NULL THEN
        ...
        VALUES 
        (gen_random_uuid()::text, v_journal_id, p_company_id, v_expense_account, COALESCE(v_bill.total, 0), 0, 'Expense for ' || v_bill_number, NOW()),
