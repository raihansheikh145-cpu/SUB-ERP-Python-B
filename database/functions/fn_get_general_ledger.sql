DROP FUNCTION IF EXISTS public.get_general_ledger_v2(text, text, date, date);
DROP FUNCTION IF EXISTS public.get_general_ledger(text, text, date, date);
...
AS $$
    ...
    BEGIN
        ...
        SELECT 
            ...
            COALESCE(c.name, c.company, '') AS contact_name,
        ...
        LEFT JOIN docs_payments p ON j.journal_type IN ('CUST_PAY', 'VEND_PAY', 'CPAY', 'VPAY') AND p.journal_entry_id = j.id
        WHERE (p_company_id IS NULL OR j.company_id = p_company_id)
          AND al.account_id = p_account_id
          AND j.status = 'POSTED'
          AND j.date >= p_start_date 
...
CREATE OR REPLACE FUNCTION public.get_general_ledger(p_company_id text, p_account_id text, p_start_date date, p_end_date date)
 RETURNS TABLE(date date, type text, ref text, description text, contact_name text, account_name text, debit numeric, credit numeric, balance numeric, is_opening boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $$
