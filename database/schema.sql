...
-- Generated: 2026-08-06T08:12:57.662Z
...
BEGIN
    ...
    IF v_is_received IS NULL THEN v_is_received := v_loan.type = 'RECEIVED'; END IF;
    
    v_total := p_principal_to_pay + p_interest_to_pay;

    v_desc := 'Loan Payment Period ' || p_period::TEXT || ': ' || COALESCE(v_loan.data->>'name', v_loan.data->>'number', v_loan.loan_number);
...
BEGIN
    ...
    IF v_type = 'RECEIVED' THEN
        ...
        IF v_loan_acc IS NULL THEN 
            ...
            VALUES (v_loan_acc, v_company_id, '210100', 'Loan Payable', 'LIABILITY', NOW());
        END IF;
    ELSE
        SELECT id INTO v_loan_acc FROM docs_accounts WHERE code = '100601' AND company_id = v_company_id LIMIT 1;
        IF v_loan_acc IS NULL THEN 
            v_loan_acc := gen_random_uuid();
            INSERT INTO docs_accounts (id, company_id, code, name, type, updated_at) 
            VALUES (v_loan_acc, v_company_id, '100601', 'Loan Receivable', 'ASSET', NOW());
        END IF;
    END IF;
