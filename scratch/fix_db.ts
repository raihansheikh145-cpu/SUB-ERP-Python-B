...
$function$;
    ...
    // We should also delete the bad cash lines from existing payment journals
    // So the Partner Ledger corrects itself!
    console.log("Fixing existing bad journal lines...");
    const res = await prisma.$executeRawUnsafe(`
      DELETE FROM docs_journal_lines
      WHERE id LIKE '%-cash' AND journal_id LIKE 'JE-LPAY-%' AND contact_id IS NOT NULL;
    `);
    console.log("Deleted " + res + " bad cash lines from previous loan payments.");
