...
files.forEach(file => {
  ...
  content = content.replace(/id:\s*([a-zA-Z0-9_]+)\?\.,\s*name:\s*\1\?\./g, 'id: \.id, name: \.name');
  content = content.replace(/id:\s*([a-zA-Z0-9_]+)\?\.,\s*label:\s*\1\?\./g, 'id: \.id, label: \.name');
  content = content.replace(/\$\{([a-zA-Z0-9_]+)\?\.\}\s*-\s*\$\{\1\?\.\}/g, '\ - \');
  content = content.replace(/p\?\.,\s*description:\s*p\?\./g, 'p?.id, description: p?.name');
  content = content.replace(/key=\{([a-zA-Z0-9_]+)\?\.\}/g, 'key={\.id}');
  content = content.replace(/([a-zA-Z0-9_]+)\?\.\s*===\s*'POSTED'/g, '\.status === \'POSTED\'');
  content = content.replace(/([a-zA-Z0-9_]+)\?\.\s*===\s*'DRAFT'/g, '\.status === \'DRAFT\'');
  content = content.replace(/([a-zA-Z0-9_]+)\?\.\s*===\s*'DELETED'/g, '\.status === \'DELETED\'');
