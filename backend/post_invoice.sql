...
AS $function$
            ...
            BEGIN
                ...
                IF v_ar_acc IS NULL THEN 
                    v_ar_acc := v_effective_company_id || '-100201';
                    INSERT INTO docs_accounts (id, company_id, code, name, type, data) VALUES (v_ar_acc, v_effective_company_id, '100201', 'Accounts Receivable', 'ASSET', jsonb_build_object('id', v_ar_acc, 'code', '100201', 'name', 'Accounts Receivable', 'type', 'ASSET', 'subType', 'ACCOUNTS_RECEIVABLE', 'companyId', v_effective_company_id)) ON CONFLICT DO NOTHING;
                END IF;
                ...
                IF v_rev_acc IS NULL THEN
                    v_rev_acc := v_effective_company_id || '-400100';
                    INSERT INTO docs_accounts (id, company_id, code, name, type, data) VALUES (v_rev_acc, v_effective_company_id, '400100', 'Sales Revenue', 'REVENUE', jsonb_build_object('id', v_rev_acc, 'code', '400100', 'name', 'Sales Revenue', 'type', 'REVENUE', 'subType', 'REVENUE', 'companyId', v_effective_company_id)) ON CONFLICT DO NOTHING;
                END IF;
                ...
                IF v_tax_acc IS NULL THEN
                    v_tax_acc := v_effective_company_id || '-200400';
                    INSERT INTO docs_accounts (id, company_id, code, name, type, data) VALUES (v_tax_acc, v_effective_company_id, '200400', 'VAT/Tax Payable', 'LIABILITY', jsonb_build_object('id', v_tax_acc, 'code', '200400', 'name', 'VAT/Tax Payable', 'type', 'LIABILITY', 'subType', 'OTHER_CURRENT_LIABILITY', 'companyId', v_effective_company_id)) ON CONFLICT DO NOTHING;
                END IF;
                ...
                IF v_cogs_acc IS NULL THEN
                    v_cogs_acc := v_effective_company_id || '-500101';
                    INSERT INTO docs_accounts (id, company_id, code, name, type, data) VALUES (v_cogs_acc, v_effective_company_id, '500101', 'Cost of Goods Sold', 'EXPENSE', jsonb_build_object('id', v_cogs_acc, 'code', '500101', 'name', 'Cost of Goods Sold', 'type', 'EXPENSE', 'subType', 'COGS', 'companyId', v_effective_company_id)) ON CONFLICT DO NOTHING;
                END IF;
                ...
                IF v_inv_acc IS NULL THEN
                    v_inv_acc := v_effective_company_id || '-100501';
                    INSERT INTO docs_accounts (id, company_id, code, name, type, data) VALUES (v_inv_acc, v_effective_company_id, '100501', 'Inventory Asset', 'ASSET', jsonb_build_object('id', v_inv_acc, 'code', '100501', 'name', 'Inventory Asset', 'type', 'ASSET', 'subType', 'INVENTORY', 'companyId', v_effective_company_id)) ON CONFLICT DO NOTHING;
                END IF;
