...
@router.get("/fix-my-role")
async def fix_my_role():
    try:
        await prisma.execute_raw("UPDATE public.auth_users SET role = 'role-superadmin' WHERE role = 'authenticated'")
        return {"status": "success", "message": "Roles updated successfully"}
    except Exception as e:
        return {"status": "error", "detail": str(e)}

@router.get("/run-jsonb-cleanup")
async def run_jsonb_cleanup():
    try:
        # docs_invoices — keep items, messages only
        ...
        # docs_payments — keep appliedInvoices, appliedBills only
        await prisma.execute_raw("""
        ...
        return {"status": "success", "message": "JSONB cleanup completed — scalar duplicates removed from data columns"}
    except Exception as e:
        return {"status": "error", "detail": str(e)}
